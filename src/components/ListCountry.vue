...
export default {
  ...
  setup() {
    var country = ref(null);

    const popupTrigger = ref({
      buttonTrigger: false,
    })

    const togglePopup = (trigger, clickedCountry) => {
...
<template>
  ...
  <div class="px-[30px] py-[30px] grid grid-cols sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-[30px]">
    ...
    <CountryDetailModal v-if="popupTrigger.buttonTrigger" :togglePopup="() => togglePopup('buttonTrigger')"
      :country="country"></CountryDetailModal>
  </div>

  <div class="container md:mb-8 pt-8 px-4 mx-auto flex justify-center select-none">
    <button :disabled="!previousBtn"
      :class="{ 'block border px-4 py-2 rounded-l hover:bg-gray-200 text-gray-600': previousBtn, 'block border px-4 py-2 rounded-l text-gray-300': !previousBtn }"
      @click="gotoPage(this.currentPage - 1)">&larr;</button>

    <button
      :class="{ 'block border px-4 py-2 hover:bg-gray-200 text-gray-600': currentPage !== 1, 'block border px-4 py-2 bg-indigo-500 hover:bg-indigo-400 text-white': currentPage === 1 }"
      v-if="searchTotalPage >= 1" @click="gotoPage(1)">1</button>
    <button
      :class="{ 'block border px-4 py-2 hover:bg-gray-200 text-gray-600': currentPage !== 2, 'block border px-4 py-2 bg-indigo-500 hover:bg-indigo-400 text-white': currentPage === 2 }"
      v-if="searchTotalPage >= 2" @click="gotoPage(2)">2</button>
    <button
      :class="{ 'block border px-4 py-2 hover:bg-gray-200 text-gray-600': currentPage !== 3, 'block border px-4 py-2 bg-indigo-500 hover:bg-indigo-400 text-white': currentPage === 3 }"
      v-if="searchTotalPage >= 3" @click="gotoPage(3)">3</button>
    <button
